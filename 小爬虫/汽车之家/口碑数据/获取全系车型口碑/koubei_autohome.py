...
class Spider:
    ...
    def get_eid(self, car):
        """获取车型口碑ID列表"""
        log_init().info(f'{car["I"]} {car["N"]} 口碑数据获取中...')
        p = 1
        while True:
            # 解析口碑ID
            url = f'https://koubei.app.autohome.com.cn/autov9.1.0/alibi/seriesalibiinfos-pm2-ss{car["I"]}-st0-p{p}-s50-isstruct0-o0.json'
            try:
            ...
            for st in range(1, 11):
                url = f'https://koubei.app.autohome.com.cn/autov9.1.0/alibi/seriesalibiinfos-pm2-ss{car["I"]}-st{st}-p{p}-s50-isstruct0-o0.json'
                response = self._parse_url(url).json()
    ...
    def get_content(self, model, eid):
        ...
        log_init().info(f'车系：{model["I"]} 口碑：{eid} 数据获取中...')
        response = self._parse_url(url).json()
        ...
        data = (userId, userName, model['I'], specid, brandname, seriesname, specname, boughtprovincename, boughtcityname,
                 dealername, boughtdate, boughtPrice, actualOilConsumption, drivekilometer, satisfaction_content,
                 Dissatisfied_content, spaceScene, space_content, powerScene, power_content, maneuverabilityScene,
                 Control_content, oilScene, oilScene_content, comfortablenessScene, Comfort_content, apperanceScene,
    ...
    def keep_records(self, model, vali=False):
        """保存获取记录"""
        file_name = '获取记录.txt'
    ...
    def run(self, model):
        # 判断是否获取
        if self.keep_records(model['I'], vali=True):
            log_init().info(f'{model["I"]} 已获取跳过!')
            return
        # 第二步 获取口碑eid
        for eid in self.get_eid(model):
            ...
            # self.scv_data(data)
            self.save_sql(data)
            log_init().info(f'车系：{model["I"]} 口碑：{eid} 数据保存成功!')
        # 保存获取记录
        log_init().info(f'{model["I"]} {model["N"]} 口碑数据获取完成!')
        self.keep_records(model["I"])
        time.sleep(1)
